...
function profilImage($images, $user)
{
    ...
    $name = getName($images['image'], $path);
...
function uploadFirmenLogo($images, $firma)
{
    ...
    $path = replaceStrToLower('images/firma/'.$firma);
    $name = getName($images, $path);

    $image = Image::make($images->getRealPath())->encode('webp', 75)->orientate()->resize(null, 90, function ($c) {
        $c->aspectRatio();
        $c->upsize();
    });
    $image->stream();
...
function numberRanges6($number, $prefix = '', $suffix = ''): string
{
    $format = $prefix.'%05d'.$suffix;
...
{
    if ($invoice->invoice_payment === '30 Tage / 2% Skonto') {
