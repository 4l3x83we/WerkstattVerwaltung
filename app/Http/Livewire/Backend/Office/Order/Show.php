...
class Show extends Component
{
    ...
    public function store()
    {
        ...
        foreach ($validatedData['invoiceDetails'] as $key => $invoiceDetail) {
            ...
            $this->order->history($this->order, $invoiceDetail);
        }
        $invoice = [
            'protocol_text' => 'Bearbeitet (Summe exkl. Steuer: '.number_format($this->order->invoice_subtotal, 2, ',', '.').'€)',
            'protocol_status' => 'edited',
        ];
        $this->order->protocol($invoice);
    ...
    public function render()
    {
        ...
        $protocols = Protocol::where('protocol_type_nr', '=', $this->order->id)
            ->orderBy('created_at', 'desc')
            ->get();

        return view('livewire.backend.office.order.show', [
            'customers' => $customers,
