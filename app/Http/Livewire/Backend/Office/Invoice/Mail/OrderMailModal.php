...
Im Anhang finden sie unsere Auftrag mit der Auftragsnummer: '.$this->invoice->order_nr.'.
...
'.$this->settings->company_email;
    ...
    public function mail()
    {
        ...
        Mail::to($this->invoice->company->comapany_email ?? 'noreplay@thueringer-tuning-freunde.de')
            ...
            ->send(new OrderMail($invoice, $mail, $settings, $pdf));
    ...
    public function pdf($rechnung)
    {
        ...
        $path = 'dokumente/'.replaceStrToLower($rechnung->customer->fullname().'/auftraege');
        if (! File::isDirectory(public_path($path))) {
            File::makeDirectory(public_path($path), 0775, true, true);
        }

        $pdf->save(public_path($path).'/Auftrag-'.$rechnung->order_nr.'.pdf');
    }
