    ...
    <div class="grid grid-cols-12 gap-4 p-4">
        <div class="col-span-12 space-y-2">
            <div class="flex justify-between">
                <span>Kassenstand laut Kassenbuch:</span>
                <span>{{ number_format($cashBook['cashBook_saldo'], 2, ',', '.') . ' €' }}</span>
            </div>
        </div>
        <div class="col-span-12 lg:col-span-6 space-y-2">
            <x-ag.forms.label for="cashBook.amount" text="Neuer Kassenstand (€)" stern="true"/>
            <x-ag.forms.input type="number" step="0.01" id="cashBook.amount" class="amount" text="Neuer Kassenstand (€)" stern="true"/>
            @push('scripts')
                <script type="module">
        ...
        <div class="col-span-12 space-y-2">
            @php
                if ($correction > 0) {
                    $class = 'text-green-800 bg-green-50 border-green-300 dark:text-green-400 dark:border-green-800';
                } elseif ($correction < 0) {
                    $class = 'text-red-800 bg-red-50 border-red-300 dark:text-red-400 dark:border-red-800';
                } else {
                    $class = 'text-blue-800 bg-blue-50 border-blue-300 dark:text-blue-400 dark:border-blue-800';
                }
            @endphp
            <div class="flex items-start p-4 mb-4 text-sm border rounded-lg dark:bg-gray-800 {{ $class }}" role="alert">
                ...
                <div class="flex items-start flex-col">
                    <span class="font-medium">
                        @if($correction > 0)
                            {{ '+ ' . number_format($correction, 2, ',', '.') . ' € Korrektur' }}<br>
                            <span class="text-gray-900 dark:text-gray-400">Es wird eine Korrekturbuchung erstellt in Höhe von {{ number_format($correction, 2, ',', '.') }} €</span>
                        @elseif($correction < 0)
                            {{ '- ' . str_replace('-', '', number_format($correction, 2, ',', '.')) . ' € Korrektur' }}<br>
                            <span class="text-gray-900 dark:text-gray-400">Es wird eine Korrekturbuchung erstellt in Höhe von {{ number_format($correction, 2, ',', '.') }} €</span>
                        @else
                            {{ '+- ' . number_format($correction, 2, ',', '.') . ' € Korrektur' }}<br>
                            <span class="text-gray-900 dark:text-gray-400">Es wird keine Korrekturbuchung erstellt</span>
                        @endif
                    </span>
                </div>
            </div>
