    ...
    <x-ag.main.head>
        ...
        <x-ag.table.table>
            ...
            <x-slot:tbody>
                    ...
                    <x-ag.table.tr class="text-sm">
                        <td class="p-2 cursor-pointer">
                            <div class="inline-flex gap-2">
                                @if($offer->printed)
                                <x-ag.button.link wire:click="print({{ $offer->id }})" class="px-2 text-white hover:text-gray-50">
                                ...
                                @else
                                <x-ag.button.link wire:click="print({{ $offer->id }})" class="px-2 text-red-500 hover:text-red-600">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0110.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0l.229 2.523a1.125 1.125 0 01-1.12 1.227H7.231c-.662 0-1.18-.568-1.12-1.227L6.34 18m11.318 0h1.091A2.25 2.25 0 0021 15.75V9.456c0-1.081-.768-2.015-1.837-2.175a48.055 48.055 0 00-1.913-.247M6.34 18H5.25A2.25 2.25 0 013 15.75V9.456c0-1.081.768-2.015 1.837-2.175a48.041 48.041 0 011.913-.247m10.5 0a48.536 48.536 0 00-10.5 0m10.5 0V3.375c0-.621-.504-1.125-1.125-1.125h-8.25c-.621 0-1.125.504-1.125 1.125v3.659M18 10.5h.008v.008H18V10.5zm-3 0h.008v.008H15V10.5z"/>
                                    </svg>
                                </x-ag.button.link>
                        ...
                        <td class="p-2 cursor-pointer" wire:click="edit({{ $offer->id }})">{{ Carbon::parse($offer->offer_date)->format('d.m.Y') }}</td>
                        <td class="p-2 cursor-pointer" wire:click="edit({{ $offer->id }})">{{ $offer->offer_nr }}</td>
                        <td class="p-2 cursor-pointer" wire:click="edit({{ $offer->id }})">{{ $offer->customer->customer_lastname }}</td>
                        <td class="p-2 cursor-pointer" wire:click="edit({{ $offer->id }})">{{ $offer->customer->customer_firstname }}</td>
                        <td class="p-2 text-right cursor-pointer" wire:click="edit({{ $offer->id }})">{{ number_format($offer->offer_subtotal, 2, ',', '.') . ' €' }}</td>
                        <td class="p-2 text-right cursor-pointer" wire:click="edit({{ $offer->id }})">{{ number_format($offer->offer_total, 2, ',', '.') . ' €' }}</td>
                        <td class="p-2 text-right cursor-pointer" wire:click="edit({{ $offer->id }})"><x-ag.badge :color="invoiceStatus($offer->offer_status)['color']">{{ invoiceStatus($offer->offer_status)['text'] }}</x-ag.badge></td>
                        <td class="p-2 text-right">
                            @can('edit')
                                <x-ag.button.link wire:click="edit({{ $offer->id }})" class="px-2 text-blue-500 hover:text-blue-600">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                            ...
                            @endcan
                            @can('delete')
                                <x-ag.button.link wire:click="$emit('triggerDelete',{{ $offer->id }})" class="px-2 text-red-500 hover:text-red-600">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                ...
                @empty
                    <x-ag.table.tr>
                        <td colspan="9" class="p-2 text-center font-bold text-lg">Es wurde noch keine Angebote
                            angelegt.
                        </td>
                    </x-ag.table.tr>
                @endforelse
