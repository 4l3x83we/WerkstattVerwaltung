                ...
                <div class="col-span-12">
                    <div class="relative overflow-x-auto overflow-y-hidden">
                        <x-ag.table.table>
                            ...
                            <x-slot:tbody>
                                @foreach($offerDetails as $index => $offerDetail)
                                    <x-ag.table.tr class="text-sm">
                                        <td class="p-2">{{ $index + 1 }}</td>
                                        <td class="p-2">
                                            @if($offerDetail['is_saved'])
                                                <x-ag.forms.input type="hidden" id="offerDetails.[{{ $index }}].product_id"/>
                                                @if($offerDetail['product_artnr'])
                                                    {{ $offerDetail['product_artnr'] }}
                                                @endif
                                                <x-ag.forms.input type="hidden" id="offerDetails.[{{ $index }}].id"/>
                                            @else
                                        ...
                                        <td class="p-2">
                                            @if($offerDetail['is_saved'])
                                                <x-ag.forms.input type="hidden" id="offerDetails.[{{ $index }}].product_name"/>
                                                @if($offerDetail['product_name'])
                                                    {{ $offerDetail['product_name'] }}
                                                @endif
                                            @else
                                        ...
                                        <td class="p-2">
                                            @if($offerDetail['is_saved'])
                                                <x-ag.forms.input type="hidden" id="offerDetails.[{{ $index }}].product_desc"/>
                                                @if($offerDetail['product_desc'])
                                                    {{ $offerDetail['product_desc'] }}
                                                @endif
                                            @else
                                        ...
                                        <td class="p-2">
                                            @if($offerDetail['is_saved'])
                                                <x-ag.forms.input type="hidden" id="offerDetails.[{{ $index }}].qty"/>
                                                @if($offerDetail['qty'])
                                                    {{ $offerDetail['qty'] }}
                                                @endif
                                            @else
                                        ...
                                        <td class="p-2 text-right">
                                            @if($offerDetail['is_saved'])
                                                <x-ag.forms.input type="hidden" id="offerDetails.[{{ $index }}].price"/>
                                                @if($offerDetail['price'])
                                                    {{ number_format($offerDetail['price'], 2) . ' €' }}
                                                @endif
                                            @else
                                        ...
                                        <td class="p-2 text-right">
                                            @if($offerDetail['is_saved'])
                                                <x-ag.forms.input type="hidden" id="offerDetails.[{{ $index }}].discountPercent"/>
                                                <x-ag.forms.input type="hidden" id="offerDetails.[{{ $index }}].discount"/>
                                                @if($offerDetail['discount'] > 0)
                                                    {{ number_format($offerDetail['discount'], 2) . ' €' }}
                                                @endif
                                            @else
                                        ...
                                        <td class="p-2 text-right">
                                            @if($offerDetail['is_saved'])
                                                <x-ag.forms.input type="hidden" id="offerDetails.[{{ $index }}].subtotal"/>
                                                @if($offerDetail['subtotal'])
                                                    {{ number_format($offerDetail['subtotal'], 2) . ' €' }}
                                                @endif
                                            @else
                                        ...
                                        <td class="p-2 text-center">
                                            @if($offerDetail['is_saved'])
                                                ...
                                                {{--                                        @elseif($offerDetail['product_artnr'])--}}
                                            @elseif($product_art_nr)
                                                <x-ag.button.link type="button" wire:click="saveProduct({{ $index }}, {{ $offerDetail['id'] ?? '' }})" class="px-2 text-green-500 hover:text-green-600">
                                    ...
                                    <x-ag.table.tr class="text-sm">
                                        <td class="p-2 align-top" colspan="5" rowspan="7">
                                            @if($toPay)
                                                ...
                                                @else
                                                    Zahlungsart: <span class="font-bold">{{ $offers['offer_payment'] }}</span>
                                                @endif
                                            @else
