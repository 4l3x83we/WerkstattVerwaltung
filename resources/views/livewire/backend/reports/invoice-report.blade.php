            ...
            <x-ag.table.table>
                <x-slot:thead>
                    ...
                    <x-ag.table.th>Rechnungsdatum
                        <x-ag.table.th-sortBy id="invoice_date" field="{{$sortField}}" direction="{{$sortDirection}}" wire:click="sortBy('invoice_date')"/>
                    </x-ag.table.th>
                    <x-ag.table.th>Fällig am
                        <x-ag.table.th-sortBy id="invoice_due_date" field="{{$sortField}}" direction="{{$sortDirection}}" wire:click="sortBy('invoice_due_date')"/>
                    </x-ag.table.th>
                    <x-ag.table.th>Status
                        <x-ag.table.th-sortBy id="invoice_payment_status" field="{{$sortField}}" direction="{{$sortDirection}}" wire:click="sortBy('invoice_payment_status')"/>
                    </x-ag.table.th>
                ...
                <x-slot:tbody>
                    @foreach($invoices as $invoice)
                        ...
                        <x-ag.table.tr class="text-sm" wire:click="show({{ $invoice->id }})">
                            ...
                            <td class="p-2 cursor-pointer">{{ Carbon::parse($invoice->invoice_date)->format('d.m.Y') }}</td>
                            <td class="p-2 cursor-pointer">{{ Carbon::parse($invoice->invoice_due_date)->format('d.m.Y') }}</td>
                            ...
                            <td class="p-2 cursor-pointer text-right">
                                    @if($invoice->payment()->sum('payment_amount') < 0)
                                        <span class="text-red-600">{{ number_format($invoice->payment()->sum('payment_amount'), 2, ',', '.') . ' €' }}</span>
                                    @elseif($invoice->payment()->sum('payment_amount') > 0)
                                        <span class="text-green-500">{{ number_format($invoice->payment()->sum('payment_amount'), 2, ',', '.') . ' €' }}</span>
                                    @else
