@error($id ?? '')
<input {{ $attributes->merge(['type' => $type ?? 'text', 'wire:model.lazy' => $id ?? '',  'id' => $id ?? '', 'class' => 'shadow-sm bg-gray-50 border border-red-300 text-gray-900 sm:text-sm rounded focus:ring-gray-500 focus:border-red-500 block w-full p-2.5 dark:bg-gray-700 dark:border-red-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-red-500', 'placeholder' => $text ?? '']) }}>
