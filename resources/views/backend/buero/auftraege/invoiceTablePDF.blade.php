                    ...
                    <table style="width: 8.5cm;">
                        ...
                        <tr>
                            <td style="height: 2.73cm; padding-left: 0.5cm; font-size: 12px; line-height: 16px;">
                                <span style="white-space: nowrap;">{{ $order->customer->customer_salutation . ' ' . $order->customer->customer_firstname . ' ' . $order->customer->customer_lastname }}</span><br />
                                <span style="white-space: nowrap;">{{ $order->customer->customer_additive }}</span><br />
                                <span style="white-space: nowrap;">{{ $order->customer->customer_street }}</span><br />
                                <span style="white-space: nowrap;">{{ $order->customer->customer_post_code . ' ' . $order->customer->customer_location }}</span><br />
                                @foreach(countryCode() as $country)
                                    <span style="white-space: nowrap;">@if($country['code'] === $order->customer->customer_country){{ $country['name'] }}@endif</span>
                                @endforeach
                            </td>
                        </tr>
                    </table>
                ...
                <td style="width: 7.4cm; padding-top: 0.5cm;">
                    <table style="font-size: 12px; line-height: 16px;">
                        <tr style="margin-bottom: 5px;">
                            ...
                            <td style="width: 50%; background-color: white; text-align: right; line-height: 16px; vertical-align: middle; font-weight: bold; white-space: nowrap;">{{ $order->order_nr }}</td>
                        </tr>
                        <tr style="margin-bottom: 5px;">
                            ...
                            <td style="width: 50%; background-color: white; text-align: right; line-height: 16px; vertical-align: middle; white-space: nowrap;">
                                {{ Carbon::parse($order->order_date)->format('d.m.Y') }}</td>
                        </tr>
                        <tr style="margin-bottom: 5px;">
                            <td style="width: 50%; vertical-align: middle;">Liefer-/Leistungsdatum:</td>
                            <td style="width: 50%; background-color: white; text-align: right; line-height: 16px; vertical-align: middle; white-space: nowrap;">
                                {{ Carbon::parse($order->delivery_performance_date)->format('d.m.Y') }}</td>
                        </tr>
                        <tr style="margin-bottom: 5px;">
                            <td style="width: 50%; vertical-align: middle;">Kundennummer:</td>
                            <td style="width: 50%; background-color: white; text-align: right; line-height: 16px; vertical-align: middle; white-space: nowrap;">{{ $order->customer->customer_kdnr }}</td>
                        </tr>
                        @if($order->customer->customer_vat_number)
                            <tr style="margin-bottom: 5px;">
                                <td style="width: 50%; vertical-align: middle;">Ihre USt-IdNr.:</td>
                                <td style="width: 50%; background-color: white; text-align: right; line-height: 16px; vertical-align: middle; white-space: nowrap;">{{ $order->customer->customer_vat_number }}</td>
                            </tr>
                        @endif
                        <tr style="margin-bottom: 5px;">
                            <td style="width: 50%; vertical-align: middle;">Sachbearbeiter/-in:</td>
                            <td style="width: 50%; background-color: white; text-align: right; line-height: 16px; vertical-align: middle; white-space: nowrap;">{{ $order->order_clerk }}</td>
                        </tr>
                    </table>
                </td>
            </tr>
...
<main id="container">
        <section id="content">
                ...
                <tr>
                    <td colspan="2" style="background-color: #e5e7eb; border-radius: 5px; padding: 5px; max-height: 1.58cm; font-size: 12px; line-height: 16px;">
                        <table>
                            <tr>
                                <td>Marke:
                                    <span style="white-space: nowrap;">{{ $order->vehicle->vehicles_brand }}</span>
                                </td>
                                <td>HSN:
                                    <span style="white-space: nowrap;">{{ $order->vehicle->vehicles_hsn }}</span>
                                </td>
                                <td>F.-Id.-Nr.:
                                    <span style="white-space: nowrap;">{{ $order->vehicle->vehicles_identification_number }}</span>
                                </td>
                            </tr>
                            <tr>
                                <td>Modell:
                                    <span style="white-space: nowrap;">{{ Str::limit($order->vehicle->vehicles_model . ' ' . $order->vehicle->vehicles_type, 25) }}</span>
                                </td>
                                <td>TSN:
                                    <span style="white-space: nowrap;">{{ $order->vehicle->vehicles_tsn }}</span>
                                </td>
                                <td>EZ:
                                    <span style="white-space: nowrap;">{{ Carbon::parse($order->vehicle->vehicles_hu)->format('d.m.Y') }}</span>
                                </td>
                            </tr>
                            <tr>
                                <td>Kennzeichen:
                                    <span style="white-space: nowrap;">{{ $order->vehicle->vehicles_license_plate }}</span>
                                </td>
                                <td>HU:
                                    <span style="white-space: nowrap;">{{ Carbon::parse($order->vehicle->vehicles_hu)->format('m.Y') }}</span>
                                </td>
                                <td>Km-Stand:
                                    <span style="white-space: nowrap;">{{ $order->vehicle->vehicles_mileage }}</span>
                                </td>
                            </tr>
                        </table>
                    </td>
            ...
            @if($order->order_notes_1)
                <table>
                    <tr>
                        <td colspan="8">
                            <div style="padding: 10px 0;">{{ $order->order_notes_1 }}</div>
                        </td>
                    </tr>
                </table>
            @endif
            <table style="margin-top: 0.5cm;" cellspacing="0" cellpadding="0">
                ...
                @foreach($orderDetails as $key => $orderDetail)
                    <tr>
                        ...
                        <td style="padding: 0 2px;">{{ $orderDetail->product->product_artnr }}</td>
                        <td style="padding: 0 2px;">{{ $orderDetail->product->product_name }} {{ $orderDetail->product->product_desc }}</td>
                        <td style="padding: 0 2px; text-align: right;">{{ $orderDetail->qty }}</td>
                        <td style="padding: 0 2px;">{{ $orderDetail->product->product_einheit }}</td>
                        <td style="padding: 0 2px;">{{ is_null($orderDetail->discount) ? $orderDetail->discountPercent . ' %' : '' }}</td>
                        <td style="padding: 0 2px; text-align: right;">{{ number_format($orderDetail->price, 2, ',', '.') . ' €' }}</td>
                        <td style="padding: 0 2px; text-align: right;">{{ number_format($orderDetail->subtotal, 2, ',', '.') . ' €' }}</td>
                    </tr>
                @endforeach
                    ...
                    <tr>
                        <td style="padding: 0 2px; border-top: 1px double #374151;" colspan="5" rowspan="7">
                            @if($toPay)
                                @if($skonto)
                                    Zahlbar bis
                                    zum {{ Carbon::parse($order->order_date)->addDays(30)->isoFormat('DD. MMMM YYYY') }}
                                    ohne Abzug.<br>
                                    Bei Zahlung
                                    bis {{ Carbon::parse($order->order_date)->addDays(14)->isoFormat('DD. MMMM YYYY') }}
                                    gewähren wir 2.00 % Skonto (= {{ number_format($skonto, 2, ',', '.') . ' €' }}) auf
                                    den
                                    Gesamtbetrag.<br>
                                    (Zahlbetrag abzüglich Skonto
                                    = {{ number_format($order->order_total - $skonto, 2, ',', '.') . ' €' }})
                                @endif
                            @else
                        ...
                        <td style="text-align: right; border-top: 1px double #374151;">{{ number_format($order->order_subtotal, 2, ',', '.') . ' €' }}</td>
                    </tr>
                @endif
                <tr>
                    @if($order->order_discount > 0)
                        ...
                        <td style="text-align: right;">
                            -{{ number_format($order->order_discount, 2, ',', '.') . ' €' }}</td>
                    @endif
                </tr>
                <tr>
                    @if($order->order_shipping > 0)
                        <td style="padding: 0 2px; text-align: right;" colspan="2">Versandkosten:</td>
                        <td style="text-align: right;">{{ number_format($order->order_shipping, 2, ',', '.') . ' €' }}</td>
                    @endif
                </tr>
                <tr>
                    @if($order->order_vat_19 > 0)
                        <td style="padding: 0 2px; text-align: right;" colspan="2">MwSt (19 %):</td>
                        <td style="text-align: right;">{{ number_format($order->order_vat_19, 2, ',', '.') . ' €' }}</td>
                    @endif
                </tr>
                <tr>
                    @if($order->order_vat_7 > 0)
                        <td style="padding: 0 2px; text-align: right;" colspan="2">MwSt (7 %):</td>
                        <td style="text-align: right;">{{ number_format($order->order_vat_7, 2, ',', '.') . ' €' }}</td>
                    @endif
                </tr>
                <tr>
                    @if($order->order_vat_at > 0)
                        <td style="padding: 0 2px; text-align: right;" colspan="2">AT-Steuer (20.9 %):</td>
                        <td style="text-align: right;">{{ number_format($order->order_vat_at, 2, ',', '.') . ' €' }}</td>
                    @endif
                </tr>
                <tr>
                    <td style="padding: 0 2px; text-align: right; font-weight: bold;" colspan="2">Gesamtbetrag:</td>
                    <td style="text-align: right; font-weight: bold;">{{ number_format($order->order_total, 2, ',', '.') . ' €' }}</td>
                </tr>
                @if($order->order_external_service > 0)
                    <tr>
                        <td colspan="5"></td>
                        <td style="padding: 0 2px; text-align: right;" colspan="2">Fremdgebühren*:</td>
                        <td style="text-align: right;">{{ number_format($order->order_external_service, 2, ',', '.') . ' €' }}</td>
                    </tr>
                @endif
                @if($toPay)
                    <tr>
                ...
                @endif
                @if($order->order_notes_2)
                    <tr>
                        <td colspan="8">
                            <div style="padding: 10px 0;">{{ $order->order_notes_2 }}</div>
                        </td>
                    </tr>
                @endif
            </table>
