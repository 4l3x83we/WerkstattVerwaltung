    ...
    <div class="grid grid-cols-1 p-4 pb-0 xl:grid-cols-3 xl:gap-4 dark:bg-gray-900">
        <div class="mb-4 col-span-full xl:mb-2">
            <div class="breadcrumbs">
                {!! Breadcrumbs::render('invoicePaidShow', $paid) !!}
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Rechnung {{ $paid->invoice_nr }}</h1>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-12 xl:gap-4 dark:bg-gray-900">
                        ...
                        <div class="grid grid-cols-1 xl:grid-cols-12 gap-4">
                            <div class="col-span-12 sm:col-span-3">
                                <div class="flex w-full items-center">
                                    ...
                                    <div class="w-1/2 text-right">{{ number_format($paid->invoice_total, 2, ',', '.') . ' €' }}</div>
                                </div>
                                <div class="flex w-full items-center">
                                ...
                                <div class="flex w-full items-center">
                                    ...
                                    <div class="w-1/2 text-right">{{ number_format($paid->calculatePayment($paymentTotal), 2, ',', '.') . ' €' }}</div>
                                </div>
                            </div>
                                ...
                                <div class="grid grid-cols-12 gap-4 mt-4">
                                    <div class="col-span-12">
                                        <div class="flex w-full space-x-4 mb-4">
                                            ...
                                            <span class="w-3/4">
                                                {{ $paid->customer->customer_firstname .' '. $paid->customer->customer_lastname }}
                                            </span>
                                        </div>
                                        <div class="flex w-full space-x-4 mb-4">
                                            <span class="w-1/4 text-right">Rechnungsadresse</span>
                                            <span class="w-3/4">
                                                Kd-Nr. {{ $paid->customer->customer_kdnr }}<br>
                                                {{ $paid->customer->customer_salutation .' '. $paid->customer->customer_firstname .' '. $paid->customer->customer_lastname }}<br>
                                                {{ $paid->customer->customer_street }}<br>
                                                {{ $paid->customer->customer_post_code .' '. $paid->customer->customer_location  }}<br>
                                                {{ $paid->customer->customer_vat_number }}
                                            </span>
                                        </div>
                                        <div class="flex w-full space-x-4 mb-4">
                                            <span class="w-1/4 text-right">Fahrzeug</span>
                                            <span class="w-3/4">
                                                {{ $paid->vehicle->vehicles_license_plate .' | '. $paid->vehicle->vehicles_brand .'  '. $paid->vehicle->vehicles_model .' '. $paid->vehicle->vehicles_type }} @if($paid->vehicle->vehicles_mileage) {{ ' | ' . $paid->vehicle->vehicles_mileage . ' km' }} @else | Kilometerstand nicht angegeben @endif
                                            </span>
                                        </div>
                                        <div class="flex w-full space-x-4 mb-4">
                                            <span class="w-1/4 text-right">Rechnungsdatum</span>
                                            <span class="w-3/4">
                                                {{ Carbon::parse($paid->invoice_date)->format('d.m.Y') }}
                                            </span>
                                        </div>
                                        <div class="flex w-full space-x-4 mb-4">
                                            <span class="w-1/4 text-right">Leistungszeitraum</span>
                                            <span class="w-3/4">
                                                 {!! $paid->checkInvoiceDateWithPerformanceDate() !!}
                                            </span>
                                        </div>
                                        @if(!is_null($paid->invoice_due_date))
                                            <div class="flex w-full space-x-4 mb-4">
                                                <span class="w-1/4 text-right">Fällig am</span>
                                                <span class="w-3/4">
                                                    {{ Carbon::parse($paid->invoice_due_date)->format('d.m.Y') }}
                                                </span>
                                            </div>
                                        @endif
                                        <div class="flex w-full space-x-4 mb-4">
                                            ...
                                            <span class="w-3/4">
                                                @if(!is_null($paid->invoice_notes_2)) {!! nl2br($paid->invoice_notes_2) !!} @else -- @endif
                                            </span>
                                        </div>
                                        <div class="flex w-full space-x-4 mb-4">
                                            <span class="w-1/4 text-right">Auftragstext</span>
                                            <span class="w-3/4">
                                                @if(!is_null($paid->invoice_notes_1)) {!! nl2br($paid->invoice_notes_1) !!} @else -- @endif
                                            </span>
                                        </div>
                                        <div class="flex w-full space-x-4 mb-4">
                ...
                <div class="col-span-2">
                    <x-ag.card.head>
                        <div class="space-y-2">
                            @if($payment_in_percent < 100)
                                @livewire('backend.office.invoice.button.edit', ['order' => $paid->invoice_nr])
                            @endif
                            @livewire('backend.office.invoice.copy.copy', ['order' => $paid])

                            @if($payment_in_percent < 100)
                                <x-ag.button.button class="w-full justify-center !text-red-700 !border-red-700 !hover:bg-red-800 !focus:ring-red-300 !dark:border-red-500 !dark:text-red-500 !dark:hover:bg-red-600 !dark:focus:ring-red-900" wire:click="remove({{ $paid->id }})">
                                    Stornieren
                                </x-ag.button.button>
                            @endif
                        </div>
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="text-xs">{{ Carbon::parse($paid->invoice_date)->format('d.m.Y') }}</span>
                        </div>
