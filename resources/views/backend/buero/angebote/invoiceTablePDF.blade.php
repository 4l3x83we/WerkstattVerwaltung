...
<header>
                    ...
                    <table style="width: 8.5cm;">
                        ...
                        <tr>
                            <td style="height: 2.73cm; padding-left: 0.5cm; font-size: 12px; line-height: 16px;">
                                <span style="white-space: nowrap;">{{ $offer->customer->customer_salutation . ' ' . $offer->customer->customer_firstname . ' ' . $offer->customer->customer_lastname }}</span><br />
                                <span style="white-space: nowrap;">{{ $offer->customer->customer_additive }}</span><br />
                                <span style="white-space: nowrap;">{{ $offer->customer->customer_street }}</span><br />
                                <span style="white-space: nowrap;">{{ $offer->customer->customer_post_code . ' ' . $offer->customer->customer_location }}</span><br />
                                @foreach(countryCode() as $country)
                                    <span style="white-space: nowrap;">@if($country['code'] === $offer->customer->customer_country){{ $country['name'] }}@endif</span>
                                @endforeach
                            </td>
                        </tr>
                    </table>
                ...
                <td style="width: 7.4cm; padding-top: 0.5cm;">
                    <table style="font-size: 12px; line-height: 16px;">
                        <tr style="margin-bottom: 5px;">
                            ...
                            <td style="width: 50%; background-color: white; text-align: right; line-height: 16px; vertical-align: middle; font-weight: bold; white-space: nowrap;">{{ $offer->offer_nr }}</td>
                        </tr>
                        <tr style="margin-bottom: 5px;">
                            ...
                            <td style="width: 50%; background-color: white; text-align: right; line-height: 16px; vertical-align: middle; white-space: nowrap;">
                                {{ Carbon::parse($offer->offer_date)->format('d.m.Y') }}</td>
                        </tr>
                        <tr style="margin-bottom: 5px;">
                            <td style="width: 50%; vertical-align: middle;">Liefer-/Leistungsdatum:</td>
                            <td style="width: 50%; background-color: white; text-align: right; line-height: 16px; vertical-align: middle; white-space: nowrap;">
                                {{ Carbon::parse($offer->delivery_performance_date)->format('d.m.Y') }}</td>
                        </tr>
                        <tr style="margin-bottom: 5px;">
                            <td style="width: 50%; vertical-align: middle;">Kundennummer:</td>
                            <td style="width: 50%; background-color: white; text-align: right; line-height: 16px; vertical-align: middle; white-space: nowrap;">{{ $offer->customer->customer_kdnr }}</td>
                        </tr>
                        @if($offer->customer->customer_vat_number)
                            <tr style="margin-bottom: 5px;">
                                <td style="width: 50%; vertical-align: middle;">Ihre USt-IdNr.:</td>
                                <td style="width: 50%; background-color: white; text-align: right; line-height: 16px; vertical-align: middle; white-space: nowrap;">{{ $offer->customer->customer_vat_number }}</td>
                            </tr>
                        @endif
                        <tr style="margin-bottom: 5px;">
                            <td style="width: 50%; vertical-align: middle;">Sachbearbeiter/-in:</td>
                            <td style="width: 50%; background-color: white; text-align: right; line-height: 16px; vertical-align: middle; white-space: nowrap;">{{ $offer->offer_clerk }}</td>
                        </tr>
                    </table>
                </td>
            </tr>
    ...
    <main id="container">
        <section id="content">
                ...
                <tr>
                    <td colspan="2" style="background-color: #e5e7eb; border-radius: 5px; padding: 5px; max-height: 1.58cm; font-size: 12px; line-height: 16px;">
                        <table>
                            <tr>
                                <td>Marke:
                                    <span style="white-space: nowrap;">{{ $offer->vehicle->vehicles_brand }}</span>
                                </td>
                                <td>HSN:
                                    <span style="white-space: nowrap;">{{ $offer->vehicle->vehicles_hsn }}</span>
                                </td>
                                <td>F.-Id.-Nr.:
                                    <span style="white-space: nowrap;">{{ $offer->vehicle->vehicles_identification_number }}</span>
                                </td>
                            </tr>
                            <tr>
                                <td>Modell:
                                    <span style="white-space: nowrap;">{{ Str::limit($offer->vehicle->vehicles_model . ' ' . $offer->vehicle->vehicles_type, 25) }}</span>
                                </td>
                                <td>TSN:
                                    <span style="white-space: nowrap;">{{ $offer->vehicle->vehicles_tsn }}</span>
                                </td>
                                <td>EZ:
                                    <span style="white-space: nowrap;">{{ Carbon::parse($offer->vehicle->vehicles_hu)->format('d.m.Y') }}</span>
                                </td>
                            </tr>
                            <tr>
                                <td>Kennzeichen:
                                    <span style="white-space: nowrap;">{{ $offer->vehicle->vehicles_license_plate }}</span>
                                </td>
                                <td>HU:
                                    <span style="white-space: nowrap;">{{ Carbon::parse($offer->vehicle->vehicles_hu)->format('m.Y') }}</span>
                                </td>
                                <td>Km-Stand:
                                    <span style="white-space: nowrap;">{{ $offer->vehicle->vehicles_mileage }}</span>
                                </td>
                            </tr>
                        </table>
                    </td>
            ...
            @if($offer->offer_notes_1)
                <table>
                    <tr>
                        <td colspan="8">
                            <div style="padding: 10px 0;">{{ $offer->offer_notes_1 }}</div>
                        </td>
                    </tr>
                </table>
            @endif
            <table style="margin-top: 0.5cm;" cellspacing="0" cellpadding="0">
                ...
                @foreach($offerDetails as $key => $offerDetail)
                    <tr>
                        ...
                        <td style="padding: 0 2px;">{{ $offerDetail->product->product_artnr }}</td>
                        <td style="padding: 0 2px;">{{ $offerDetail->product->product_name }} {{ $offerDetail->product->product_desc }}</td>
                        <td style="padding: 0 2px; text-align: right;">{{ $offerDetail->qty }}</td>
                        <td style="padding: 0 2px;">{{ $offerDetail->product->product_einheit }}</td>
                        <td style="padding: 0 2px;">{{ is_null($offerDetail->discount) ? $offerDetail->discountPercent . ' %' : '' }}</td>
                        <td style="padding: 0 2px; text-align: right;">{{ number_format($offerDetail->price, 2, ',', '.') . ' €' }}</td>
                        <td style="padding: 0 2px; text-align: right;">{{ number_format($offerDetail->subtotal, 2, ',', '.') . ' €' }}</td>
                    </tr>
                @endforeach
                @if($offer->offer_subtotal > 0)
                    <tr>
                        <td style="padding: 0 2px; border-top: 1px double #374151;" colspan="5" rowspan="7">
                            @if($toPay)
                                @if($skonto)
                                    Zahlbar bis
                                    zum {{ Carbon::parse($offer->offer_date)->addDays(30)->isoFormat('DD. MMMM YYYY') }}
                                    ohne Abzug.<br>
                                    Bei Zahlung
                                    bis {{ Carbon::parse($offer->offer_date)->addDays(14)->isoFormat('DD. MMMM YYYY') }}
                                    gewähren wir 2.00 % Skonto (= {{ number_format($skonto, 2, ',', '.') . ' €' }}) auf
                                    den
                                    Gesamtbetrag.<br>
                                    (Zahlbetrag abzüglich Skonto
                                    = {{ number_format($offer->offer_total - $skonto, 2, ',', '.') . ' €' }})
                                @endif
                            @else
                        ...
                        <td style="text-align: right; border-top: 1px double #374151;">{{ number_format($offer->offer_subtotal, 2, ',', '.') . ' €' }}</td>
                    </tr>
                @endif
                <tr>
                    @if($offer->offer_discount > 0)
                        <td style="padding: 0 2px; text-align: right;" colspan="2">Rabatt:</td>
                        <td style="text-align: right;">
                            -{{ number_format($offer->offer_discount, 2, ',', '.') . ' €' }}</td>
                    @endif
                </tr>
                <tr>
                    @if($offer->offer_shipping > 0)
                        <td style="padding: 0 2px; text-align: right;" colspan="2">Versandkosten:</td>
                        <td style="text-align: right;">{{ number_format($offer->offer_shipping, 2, ',', '.') . ' €' }}</td>
                    @endif
                </tr>
                <tr>
                    @if($offer->offer_vat_19 > 0)
                        <td style="padding: 0 2px; text-align: right;" colspan="2">MwSt (19 %):</td>
                        <td style="text-align: right;">{{ number_format($offer->offer_vat_19, 2, ',', '.') . ' €' }}</td>
                    @endif
                </tr>
                <tr>
                    @if($offer->offer_vat_7 > 0)
                        <td style="padding: 0 2px; text-align: right;" colspan="2">MwSt (7 %):</td>
                        <td style="text-align: right;">{{ number_format($offer->offer_vat_7, 2, ',', '.') . ' €' }}</td>
                    @endif
                </tr>
                <tr>
                    @if($offer->offer_vat_at > 0)
                        <td style="padding: 0 2px; text-align: right;" colspan="2">AT-Steuer (20.9 %):</td>
                        <td style="text-align: right;">{{ number_format($offer->offer_vat_at, 2, ',', '.') . ' €' }}</td>
                    @endif
                </tr>
                <tr>
                    <td style="padding: 0 2px; text-align: right; font-weight: bold;" colspan="2">Gesamtbetrag:</td>
                    <td style="text-align: right; font-weight: bold;">{{ number_format($offer->offer_total, 2, ',', '.') . ' €' }}</td>
                </tr>
                @if($offer->offer_external_service > 0)
                    <tr>
                        <td colspan="5"></td>
                        <td style="padding: 0 2px; text-align: right;" colspan="2">Fremdgebühren*:</td>
                        <td style="text-align: right;">{{ number_format($offer->offer_external_service, 2, ',', '.') . ' €' }}</td>
                    </tr>
                @endif
                @if($toPay)
                    <tr>
                ...
                @endif
                @if($offer->offer_notes_2)
                    <tr>
                        <td colspan="8">
                            <div style="padding: 10px 0;">{{ $offer->offer_notes_2 }}</div>
                        </td>
                    </tr>
                @endif
            </table>
